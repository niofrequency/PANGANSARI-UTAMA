...
interface LaundryStaffPortalProps {
  ...
  // Scan-to-Job (deepLink.ts): true when a "laundry" job QR is what got
  // this Laundry Staff here — the QR carries no site (see deepLink.ts's
  // file header), so there's nothing to block on; onDeepLinkHandled just
  // clears the pending job once it's actually been submitted.
  fromQr?: boolean;
...
export function LaundryStaffPortal({ store, fromQr, onDeepLinkHandled, onScanJob }: LaundryStaffPortalProps) {
  ...
  // Reopening one of this Laundry Staff's own History entries to edit and
  ...
  // picker like LaundryShopForm does — Home Site is close enough for a
